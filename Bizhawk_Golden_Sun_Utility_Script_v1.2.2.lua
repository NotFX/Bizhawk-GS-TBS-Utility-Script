...
function multMod (A,B,C) -- Multiplies big numbers, and takes mod 2^32
  ...
  local a2 = bit.band(A,0xFFFF)
  local b1 = bit.rshift(B,16)
  local b2 = bit.band(B,0xFFFF)
  local p = bit.band(bit.lshift(a1*b2 + a2*b1,16) + a2*b2 + C, -1)
...
function RNC(R)   -- RN concatenate
    return bit.band(bit.rshift(R, 8), 0xFFFF)
end
...
function RNA (R, advances) -- RN Advance Function
    if advances == nil then 
        ...
        advances = bit.band(advances, -1)
        for i=1,32 do
            if bit.band(advances, 1)==1 then
            R = multMod(R, RNmultipliers[i], RNintervals[i])
            end
            advances = bit.rshift(advances, 1)
...
function RNB (R) -- RN Advance and reduce for use in RNG calculations
    return bit.band(bit.rshift(RNA(R), 8), 0xFFFF)
end
...
if MapDataCheck == true and bit.band(bit.rshift(memory.readbyte(0x02000060),3),1) ~= 1 then -- if MapData plugin is installed then
    ...
    function extract_height(S)
        S = bit.rshift(memory.read_u32_le(S),24)
        S = 0x202C000+S*4
